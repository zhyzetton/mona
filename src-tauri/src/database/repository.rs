...
fn row_to_media(row: &rusqlite::Row) -> Result<Media, rusqlite::Error> {
...
pub fn get_by_id(conn: &Connection, video_id: i64) -> Result<Media> {
    let mut stmt = conn.prepare(
        "SELECT \
            id, title, year, overview, media_type, duration, \
            rating, actors, poster_path, detail_img_path, file_path, file_size, resolution \
         FROM media \
         WHERE id = ?1",
    )?;
...
pub fn insert_media_with_tx(tx: &Transaction, media: &Media) -> Result<i64> {
    tx.execute(
        "INSERT INTO media (title, year, overview, media_type, duration, rating, \
         actors, poster_path, detail_img_path, file_path, file_size, resolution) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (
            &media.title,
            &media.year,
            &media.overview,
